...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Slide 2
	...
	Planck Satellite observations give the constraints \cite{aghanim2020planck}:
	\begin{center}
		...
		\flushright \textit{G. Chapline} (1975)\cite{chapline1975cosmological}.
	...
	\begin{frame}
		...
		\end{figure}
	\begin{itemize}
		\item Large inhomogeneities needed: \(\delta \ge \delta _c  \sim w = \frac{1}{3}\) \cite{carr1975primordial}.
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{center}
			{\small\(\pumpkin\) A monochromatic mass function is assumed~\cite{carr2020constraints}.}
		\end{center}
...
\begin{columns}
	...
	\textbf{General assumptions~\cite{carr2020primordial}}
	\begin{itemize}
...
\begin{frame}
	\begin{columns}
		...
		\textbf{Dynamical constraints}:\cite{carr2020constraints}
	...
	\textbf{Accretion constraints:}\cite{carr2020primordial}
...
\begin{frame}
	\frametitle{PBH binary formation \cite{raidal2017gravitational}}
	\begin{columns}
		\column{0.7\textwidth}
...
{\small	\(dn_3(x,y) = \frac{1}{2}\left(\rho_{DM}\frac{\psi(m)}{m}dm_1\right)\left(e^{-N(y)} dN(x,m_2)dN(y,m_3) \right))\)
	...
	\begin{itemize}
		...
		\item Subdominant unless \(\delta_{dc}\) very high.
	\end{itemize}
	\end{columns}
...
\begin{frame}
	\frametitle{LIGO/Virgo observations \cite{raidal2017gravitational}}
...
%		\caption{\tiny Microlensing events with mass estimates in the mass gap. Top panel: Light curves. Bottom panel: Mass-distance posterior probability densities along with the 95\% confidence level contour.}
	\end{figure}
	Among others OGLE\cite{wyrzykowski2020constraining} has detected lensing events probably due to black holes, whose mass function peaks between \(0.8\) and \(5M_{\odot}\), overlapping with the mass gap of \(2-5M_{\odot}\) where black holes are not expected to form via gravitational collapse.
\end{frame}

\begin{frame}
	\frametitle{Ultra Faint Dwarf Galaxies \cite{clesse2018seven}}
	\begin{figure}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item {\small Raidal \textit{et al.} \cite{raidal2017gravitational} show that PBHs can explain LIGO/Virgo observations without violating any current constraints, if they have a lognormal distribution function. They predict a gravitational wave background that can be observed by coming runs of LIGO.}
		\end{itemize}
	\end{columns}
\end{frame}
...
\begin{frame}
\frametitle{Density Contrast threshold \cite{carr1975primordial}}
Spherically symmetric perturbation:
\[
ds^2 = d\tau^2 - S^2\left[\frac{dr^2}{1-\kappa r^2} + r^2d\Omega^2\right]
\implies 
\begin{cases}
t_c \sim t_0\delta_0^{-\frac{3(1+w)}{2(1+3w)}}\\
S_c \sim R_0\delta_0^{-\frac{1}{(1+3w)}}
\end{cases}
\]
