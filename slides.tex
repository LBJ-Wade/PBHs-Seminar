...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Slide 2
	...
	\end{center}
		...
		\begin{block}{Mass \& time of formation \cite{hawking1971gravitationally}}
			...
			\[
			M_{PBH} \sim \frac{c^3 t}{G} \sim 10^{15} \left(\frac{t}{10^{-23} \si{\second}}\right) \si{\g}.
			\]
		\end{block}
	...
	\subsection{Formation from large inhomogeneities}
	\begin{frame}
	...
	\begin{itemize}
		...
		\item \(P(\delta)\) is gaussian \(\implies\) \(\beta(M) \sim exp\left(-\frac{\delta_c^2}{2\sigma(M)^2}\right) \ll 1\).
...
\begin{frame}
	...
	\begin{figure}
		\includegraphics[scale=0.17]{Immagini/Lensing.png}
...
\begin{frame}
	\frametitle{Ultra Faint Dwarf Galaxies}
	\begin{figure}
		\includegraphics[scale=0.22]{Immagini/UFDG.png}
		\caption{\tiny Absolute magnitude vs. the half-light ration of dwarf galaxies. Mass-to-light ratio based on toy model with uniform spherical halo with mass, radius and virial velocity relations calibrated on Eridanus II.\cite{Clesse:2017bsw}}
	\end{figure}
